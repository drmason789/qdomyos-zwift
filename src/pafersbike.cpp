...
void pafersbike::writeCharacteristic(uint8_t *data, uint8_t data_len, const QString &info, bool disable_log,
...
void pafersbike::forceResistance(resistance_t requestResistance) {
    uint8_t noOpData[] = {0x55, 0x11, 0x01, 0x18};
...
double pafersbike::GetWattFromPacket(const QByteArray &packet) {
    uint16_t convertedData = (packet.at(8) << 8) | ((uint8_t)packet.at(9));
...
void pafersbike::stateChanged(QLowEnergyService::ServiceState state) {
    ...
    if (state == QLowEnergyService::ServiceDiscovered) {
        ...
        connect(gattCommunicationChannelService, &QLowEnergyService::characteristicChanged, this,
                &pafersbike::characteristicChanged);
        connect(gattCommunicationChannelService, &QLowEnergyService::characteristicWritten, this,
                &pafersbike::characteristicWritten);
        connect(gattCommunicationChannelService,
                static_cast<void (QLowEnergyService::*)(QLowEnergyService::ServiceError)>(&QLowEnergyService::error),
                this, &pafersbike::errorService);
...
void pafersbike::descriptorWritten(const QLowEnergyDescriptor &descriptor, const QByteArray &newValue) {
...
void pafersbike::characteristicWritten(const QLowEnergyCharacteristic &characteristic, const QByteArray &newValue) {
    Q_UNUSED(characteristic);
    qDebug() << QStringLiteral("characteristicWritten ") + newValue.toHex(' ');
}

void pafersbike::serviceScanDone(void) {
...
void pafersbike::errorService(QLowEnergyService::ServiceError err) {
    ...
    qDebug() << QStringLiteral("pafersbike::errorService") + QString::fromLocal8Bit(metaEnum.valueToKey(err)) +
                    m_control->errorString();
}

void pafersbike::error(QLowEnergyController::Error err) {
    QMetaEnum metaEnum = QMetaEnum::fromType<QLowEnergyController::Error>();
    qDebug() << QStringLiteral("pafersbike::error") + QString::fromLocal8Bit(metaEnum.valueToKey(err)) +
                    m_control->errorString();
}

void pafersbike::deviceDiscovered(const QBluetoothDeviceInfo &device) {
                    ...
                    device.address().toString() + ')';
    {
        ...
        connect(m_control, &QLowEnergyController::serviceDiscovered, this, &pafersbike::serviceDiscovered);
        connect(m_control, &QLowEnergyController::discoveryFinished, this, &pafersbike::serviceScanDone);
        connect(m_control,
                static_cast<void (QLowEnergyController::*)(QLowEnergyController::Error)>(&QLowEnergyController::error),
                this, &pafersbike::error);
        connect(m_control, &QLowEnergyController::stateChanged, this, &pafersbike::controllerStateChanged);
...
void pafersbike::controllerStateChanged(QLowEnergyController::ControllerState state) {
